...
// --- ALTERAÇÃO AQUI: LÓGICA DE NORMALIZAÇÃO INTELIGENTE ---
function normalizeSize(input) {
  // 1. Limpa o texto: minúsculo e remove palavras como 'aro', 'pneu' e símbolos
  const cleanInput = String(input || "")
    .toLowerCase()
    .replace(/aro|pneu|medida/g, "") 
    .replace(/[^0-9r]/g, " ")       
    .trim();

  // 2. Regex para capturar: Largura (3 dígitos), Perfil (2 dígitos) e Aro (2 dígitos)
  const match = cleanInput.match(/(\d{3})\s*(\d{2})\s*r?\s*(\d{2})/);
  ...
  if (match) {
    ...
    // Formato que o motor de busca da TireStore melhor identifica
    return `${largura}/${perfil}R${aro}`;
  }

  // Se o usuário digitar algo estranho, remove apenas espaços extras
  return input.trim().replace(/\s+/g, "+");
}

function buildSearchUrl(size) {
  // Já recebe o tamanho normalizado pela lógica do webhook
  return `https://www.tirestore.com.br/pesquisa?t=${encodeURIComponent(size)}`;
}
  ...
  try {
    ...
    const products = await page.evaluate(() => {
      ...
      const isProductUrl = (url) => {
        ...
        return (u.includes("/p/") || u.includes("/produto")) && !u.includes("/pesquisa") && !u.includes("filtro");
      };
